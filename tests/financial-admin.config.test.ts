...
describe("financial administration safeguards", () => {
  it("requires MFA-backed financial administrator access and exposes only bounded read models", () => {
    ...
    expect(server).toMatch(/app\.get\(\s*"\/api\/admin\/finance\/overview"/);
    expect(database).toContain(
      "FROM mojaloop_transfers ORDER BY updated_at DESC LIMIT 100",
    );
    expect(database).toContain(
      "ledger_consistent = FALSE ORDER BY created_at DESC LIMIT 100",
    );
  });
  ...
  it("keeps recovery scenarios non-production, allowlisted, authenticated, and auditable", () => {
    ...
    expect(server).toContain(
      "Only an explicitly configured isolated non-production executor",
    );
  });
  ...
  it("bounds financial record filters and retains evidence-backed health and alert history", () => {
    ...
    expect(server).toMatch(/app\.get\(\s*"\/api\/admin\/finance\/alerts"/);
  ...
  it("records allowed alert actions, preserves downtime details, and exports only the active bounded report scope", () => {
    ...
    const migration = readFileSync(
      resolve(
        root,
        "drizzle/0020_financial_alert_actions_and_health_details.sql",
      ),
      "utf8",
    );
    expect(server).toMatch(
      /app\.post\(\s*"\/api\/admin\/finance\/alerts\/:id\/actions"/,
    );
    expect(server).toMatch(/app\.get\(\s*"\/api\/admin\/finance\/report\.csv"/);
