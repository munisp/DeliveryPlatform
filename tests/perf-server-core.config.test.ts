...
describe("W2b perf gates: compression + fast health (source contract)", () => {
  it("compression is a declared dependency", () => {
    const pkg = JSON.parse(source("package.json")) as {
      dependencies?: Record<string, string>;
    };
    expect(pkg.dependencies?.compression).toBeTruthy();
  });

  it("wires compression() immediately after httpMetricsMiddleware with an SSE exclusion", () => {
    ...
    const compressionAt = index.indexOf("app.use(\n  compression({");
    expect(metricsAt).toBeGreaterThan(-1);
    expect(compressionAt).toBeGreaterThan(metricsAt);
    // The realtime tracking SSE stream must be excluded from compression.
    expect(index).toContain("/api/tracking/live/");
    expect(index).toMatch(/filter:\s*\(req, res\)/);
    expect(index).toContain("compression.filter(req, res)");
  });
