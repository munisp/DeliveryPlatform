...
function TenantBrandingPanel({ onSaved }: { onSaved?: () => void }) {
  ...
  const [ownershipRangePreset, setOwnershipRangePreset] = useState<"all" | "last-seven-days" | "last-thirty-days" | "custom">("all");
  ...
  return <section className="tenant-branding" aria-labelledby="tenant-branding-title">
    ...
    <section className="tenant-brand-shared-presets" aria-labelledby="preset-ownership-history-title"><div><h3 id="preset-ownership-history-title">Preset ownership history</h3><p>Transfers are retained for tenant administrators as an ownership audit trail.</p></div><div className="tenant-audit-range-presets" role="group" aria-label="Ownership audit date ranges"><span>Quick range</span><button type="button" aria-pressed={ownershipRangePreset === "all"} onClick={() => { setOwnershipRangePreset("all"); setOwnershipStartDate(""); setOwnershipEndDate(""); }}>All time</button><button type="button" aria-pressed={ownershipRangePreset === "last-seven-days"} onClick={() => { const range = recentDateRange(7); setOwnershipRangePreset("last-seven-days"); setOwnershipStartDate(range.startDate); setOwnershipEndDate(range.endDate); }}>Last 7 days</button><button type="button" aria-pressed={ownershipRangePreset === "last-thirty-days"} onClick={() => { const range = recentDateRange(30); setOwnershipRangePreset("last-thirty-days"); setOwnershipStartDate(range.startDate); setOwnershipEndDate(range.endDate); }}>Last 30 days</button><button type="button" aria-pressed={ownershipRangePreset === "custom"} onClick={() => { const range = recentDateRange(30); setOwnershipRangePreset("custom"); setOwnershipStartDate(range.startDate); setOwnershipEndDate(range.endDate); }}>Custom range</button></div><div className="flex flex-wrap gap-3 py-3"><label className="grid gap-1 text-sm text-slate-300"><span>From</span><input type="date" value={ownershipStartDate} onChange={(event) => { setOwnershipRangePreset("custom"); setOwnershipStartDate(event.target.value); }} className="rounded-lg border border-slate-700 bg-slate-950 px-2 py-1" /></label><label className="grid gap-1 text-sm text-slate-300"><span>To</span><input type="date" value={ownershipEndDate} onChange={(event) => { setOwnershipRangePreset("custom"); setOwnershipEndDate(event.target.value); }} className="rounded-lg border border-slate-700 bg-slate-950 px-2 py-1" /></label></div>{ownershipStartDate && ownershipEndDate && ownershipStartDate > ownershipEndDate ? <p role="alert" className="text-sm text-rose-300">The start date must be before the end date.</p> : ownershipHistory.isLoading ? <p className="text-sm text-slate-400">Loading ownership history…</p> : ownershipHistory.data?.history.length ? <ul className="tenant-preset-list">{ownershipHistory.data.history.map((entry) => <li key={entry.id}><div><strong>{entry.presetName}</strong><small>{entry.fromOperatorEmail ?? "Previous owner"} → {entry.toOperatorEmail ?? "Current owner"} · {new Date(entry.transferredAt).toLocaleString()}</small></div></li>)}</ul> : <p className="text-sm text-slate-400">No preset ownership transfers match this date range.</p>}</section>
