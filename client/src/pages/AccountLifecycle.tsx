...
type TenantBrandingPreset = TenantBranding & { id: string; name: string; createdAt: string; updatedAt: string };
...
function TenantBrandingPanel({ onSaved }: { onSaved?: () => void }) {
  ...
  const removePreset = useMutation({
    mutationFn: (presetId: string) => request<{ deleted: boolean }>(`/api/auth/tenant-branding/presets/${presetId}`, {}),
  ...
  return <section className="tenant-branding" aria-labelledby="tenant-branding-title">
    ...
    <section className="tenant-brand-presets" aria-labelledby="tenant-brand-presets-title"><div><h3 id="tenant-brand-presets-title">Branding presets</h3><p>Save this tenant’s current colors and logo for fast, tenant-scoped reuse.</p></div><div className="tenant-preset-save"><Field id="branding-preset-name" label="Preset name" value={presetName} onChange={setPresetName} placeholder="Night operations" /><ActionButton pending={savePreset.isPending} disabled={!presetName.trim()} onClick={() => savePreset.mutate()}>Save preset</ActionButton></div>{savePreset.isError ? <p className="text-sm text-rose-300">{readableError(savePreset.error)}</p> : null}{presets.isLoading ? <p className="text-sm text-slate-400">Loading presets…</p> : presets.data?.presets.length ? <ul className="tenant-preset-list">{presets.data.presets.map((preset) => <li key={preset.id}><span className="tenant-preset-swatch" style={{ background: preset.primaryColor }} aria-hidden="true" /><div><strong>{preset.name}</strong><small>{preset.primaryColor} · {preset.accentColor}</small></div><button type="button" onClick={() => applyPreset.mutate(preset.id)} disabled={applyPreset.isPending}>Apply</button><button type="button" className="tenant-preset-delete" onClick={() => removePreset.mutate(preset.id)} disabled={removePreset.isPending}>Delete</button></li>)}</ul> : <p className="text-sm text-slate-400">No saved presets yet.</p>}</section>
