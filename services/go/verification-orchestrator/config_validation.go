...
// orchestrator cannot operate correctly without. The static manifest
// contract check (scripts/testing/check-config-contract.py) reads this
// table to cross-check the Kubernetes manifests against the code.
var requiredEnvironmentVariables = []string{
	"DATABASE_URL",
	"INTERNAL_SERVICE_TOKEN",
	"VERIFICATION_INTELLIGENCE_URL",
	"VERIFICATION_POLICY_URL",
}

// validateBootConfiguration fails fast at startup, naming every missing or
// invalid required environment variable in a single error, instead of
// exiting on the first missing variable and hiding the rest.
