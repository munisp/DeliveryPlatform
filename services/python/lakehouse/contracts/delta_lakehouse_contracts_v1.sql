...
-- Alternative target engine: open-source Delta Lake with a compatible Spark
-- catalog and S3-compatible object storage. Choose this OR the Iceberg
-- contract set for a deployment; do not dual-write both formats by default.
-- These are non-authoritative analytical tables only.

CREATE DATABASE IF NOT EXISTS lakehouse_bronze;
CREATE DATABASE IF NOT EXISTS lakehouse_silver;
CREATE DATABASE IF NOT EXISTS lakehouse_governance;

CREATE TABLE IF NOT EXISTS lakehouse_bronze.vehicle_tracker_event_v1 (
...
CREATE TABLE IF NOT EXISTS lakehouse_bronze.tigerbeetle_settlement_event_v1 (
  event_id STRING NOT NULL,
  source_system STRING NOT NULL,
...
CREATE TABLE IF NOT EXISTS lakehouse_bronze.medusa_inventory_event_v1 (
  event_id STRING NOT NULL,
...
CREATE TABLE IF NOT EXISTS lakehouse_silver.mobility_training_features_v1 (
...
CREATE TABLE IF NOT EXISTS lakehouse_governance.ml_training_run_v1 (
...
USING DELTA
PARTITIONED BY (DATE(training_started_at))
TBLPROPERTIES (
  'delta.appendOnly'='true',
