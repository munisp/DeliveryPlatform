...
export const ENV = {
  ...
  permifySchemaVersion: process.env.PERMIFY_SCHEMA_VERSION ?? "switchos-v1",
		...
		requireMfaForPrivilegedActions: parseBoolean(process.env.REQUIRE_MFA_FOR_PRIVILEGED_ACTIONS, process.env.NODE_ENV === "production"),
  ...
  developerWebhookDispatchEnabled: parseBoolean(process.env.ENABLE_DEVELOPER_WEBHOOK_DISPATCH, false),
  developerWebhookSecretRefsJson: process.env.DEVELOPER_WEBHOOK_SECRET_REFS_JSON?.trim() ?? "",
  medusaEventIngressEnabled: parseBoolean(process.env.ENABLE_MEDUSA_EVENT_INGRESS, false),
  medusaStoreWebhookSecretsJson: process.env.MEDUSA_STORE_WEBHOOK_SECRETS_JSON?.trim() ?? "",
