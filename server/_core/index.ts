...
import { assertWorkOrderTenant, createGeofence, createServiceZone, createWebhookSubscription, createWorkflowDefinition, createWorkOrder, deliverDueOperationalWebhooks, listOperationsSnapshot, logisticsErrorStatus, publishWorkflowDefinition, recordGeofenceEventsForPosition, recordTrackingPosition, transitionWorkOrder } from "./logisticsOperationsStore";
import { ingestPartnerEvent, listPartnerClients, partnerErrorStatus, registerPartnerClient, revokePartnerCredential } from "./partnerIntegrationStore";
import { createInvoice, decideDispute, financialOperationsErrorStatus, generateDueReports, listFinancialOperations, openDispute, requestReport, transitionInvoice } from "./financialOperationsStore";
...
app.use(express.json({ limit: ENV.apiBodyLimit, verify: (req, _res, buffer) => { (req as AppRequest).rawBody = Buffer.from(buffer); } }));
