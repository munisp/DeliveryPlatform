...
export type FinancialAdminSettings = { autoEscalationEnabled: boolean; autoEscalationMinutes: number; onCallWebhooks: string[]; approvedWebhookHosts: string[]; healthRetentionDays: number; updatedAt: string };
...
export async function updateFinancialAdminSettings(input: Omit<FinancialAdminSettings, "updatedAt"> & { actorId: number }): Promise<FinancialAdminSettings> {
  ...
  return { autoEscalationEnabled: Boolean(row.auto_escalation_enabled), autoEscalationMinutes: Number(row.auto_escalation_minutes), onCallWebhooks: Array.isArray(row.on_call_webhooks) ? row.on_call_webhooks : [], approvedWebhookHosts: Array.isArray(row.approved_webhook_hosts) ? row.approved_webhook_hosts : [], healthRetentionDays: Number(row.health_retention_days), updatedAt: new Date(row.updated_at).toISOString() };
}
